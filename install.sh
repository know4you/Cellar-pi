...
for required_file in "${required_files[@]}"; do
    [[ -s "$TEMP_DIR/repository/$required_file" ]] ||
...
/usr/bin/install -m 755 "$TEMP_DIR/repository/cellar_logger.py" \
    "$INSTALL_DIR/cellar_logger.py"
/usr/bin/install -m 755 "$TEMP_DIR/repository/cellar_config.py" \
    "$INSTALL_DIR/cellar_config.py"
/usr/bin/install -m 755 "$TEMP_DIR/repository/daily_report.py" \
    "$INSTALL_DIR/daily_report.py"
/usr/bin/install -m 755 "$TEMP_DIR/repository/setup.sh" \
